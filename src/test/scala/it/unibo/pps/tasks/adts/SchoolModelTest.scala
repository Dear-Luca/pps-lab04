...
class SchoolModelTest:
  val s = School(Cons(course("Math"), Nil()), Cons(teacher("Mario"), Nil()), Cons((teacher("Mario"), course("Math")), Nil()))
  ...
  @Test def testGetCourses(): Unit =
    val courses = Cons(course("Math"), Cons(course("DSA"), Cons(course("Algebra"), Nil())))
    val school = School(courses, Nil(), Nil())
    assertEquals(courses, school.courses)

  @Test def testGetTeachers(): Unit =
    val teachers = Cons(teacher("Mario"), Cons(teacher("Luigi"), Cons(teacher("Pippo"), Nil())))
    val school: School = School(Nil(), teachers, Nil())
  ...
  @Test def testSetTeacherToCourseDuplicates(): Unit =
    val newSchool = s.setTeacherToCourse(teacher("Mario"), course("Math"))
    assertEquals(s, newSchool)

  @Test def testCoursesOfATeacher(): Unit =
    assertEquals(Cons(course("Math"), Nil()), s.coursesOfATeacher(teacher("Mario")))
